...
<h1>查询标签: <span class="btn btn-primary"><?=$tag->name;?></span></h1><br/><br/>
<a class="btn btn-warning" href="<?=Yii::$app->request->getReferrer()?>">返回上一页</a>
<p>
...
<?php foreach ($filterData as $key=>$data):?>
    ...
    <div class="col-md-3">
        <div class="panel panel-info">
            ...
            <div class="panel-footer"><div><a href="<?=Yii::$app->params['videoServer'].$data['origin_url'];?>" class="playVideo">播放</a> | <a href="<?= \yii\helpers\Url::to(['detail','id'=>$data['id']])?>">详情</a></div></div>
        </div>
    </div>
    <?php if(($key+1)%4 ==0):?>
        </div>
